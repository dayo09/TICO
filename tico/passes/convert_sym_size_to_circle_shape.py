...
class ConvertSymSizeToCircleShape(PassBase):
    ...
    The circle_custom::shape operator is similar to TensorFlow's shape operator and
    allows preserving dynamic shape information in the Circle model. This is essential
    for models with dynamic batch sizes or other dynamic dimensions.
    ...
    Example:
        ...
        After:  %shape_0 = call_function[target=torch.ops.circle_custom.shape](args=(%x,))
                ...
                %squeeze_0 = call_function[target=torch.ops.aten.squeeze.dim](args=(%slice_0, 0))
    """
    ...
    def call(self, exported_program: ExportedProgram) -> PassResult:
        ...
        for node in graph.nodes:
            ...
            if node.target == torch.ops.aten.sym_size.int:
                ...
                # Extract the specific dimension using slice + squeeze
                # shape is 1D, so we slice [dim:dim+1] then squeeze dim 0
                ...
                # logger.debug(
                #     f"Converted {node.name} (sym_size.int) to {shape_node.name} (circle_custom::shape) + {slice_node.name} (slice) + {squeeze_node.name} (squeeze)"
                # )
