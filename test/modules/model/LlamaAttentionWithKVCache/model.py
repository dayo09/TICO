...
class LlamaAttentionWithKVCache(torch.nn.Module):
    def __init__(self):
        ...
        self.model = LlamaAttention(
            config= self.config, layer_idx = 0
        ).to("cpu")
        self.rtol = 1e-4
        self.atol = 1e-4
    ...
    def get_example_inputs(self):
        ...
        position_embeddings = (torch.randn(1, seq_len, head_dim), torch.randn(1, seq_len, head_dim))
        # attention_mask의 causal_mask 버젼: (batch_size, 1, query_length, key_value_length)
        attention_mask = torch.Tensor([[[[0.]] * seq_len]]) # shape: 1, 1, seq_len, 1
        prev_seq_len = 4
        past_key_values = DynamicCache()
        past_key_values.update(torch.randn(1, num_heads, prev_seq_len, head_dim), torch.randn(1, num_heads, prev_seq_len, head_dim), 0)
